package com.vector.uiforlife.logger;
...
public class Logger {
    ...
    public final static boolean debug = true; //是否打印log，在其他地方使用Log 打印的时候
    public final static boolean urlTest = false; //是否启动Url测试

	/**
	 * log tag
	 */
	private String tagName = "uiforlife";// tag store_employee_name
...
//	private static int logLevel = Log.ASSERT;
	 ...
	 * log.s
	 */
	public void s(String format, Object... args) {
		if (logLevel <= Log.WARN) {
			lock.lock();
			try {
				String message = createMessage(getInputString(format, args));
				Log.w(tagName+"_s", message);
			} finally {
				lock.unlock();
			}
		}
